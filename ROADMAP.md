...
1. **Initialisation du projet** 🟢
   ...
   - [x] Configurer le **système de versions** (Git) et la **CI/CD** (GitHub Actions, GitLab CI…)  
   - [x] Mettre en place un **Docker Compose** (si nécessaire) pour la base de données + application  

2. **Configuration de la base de données** 🟢
   - [x] Choisir la DB (PostgreSQL, MySQL, etc.)  
   - [x] Écrire la config de connexion (fichiers `.env`)  
   - [x] Lancer une migration initiale (ex. table `users`)  

3. **Gestion de l'authentification** (côté client ET côté boulangerie/admin) 🟢
   - [x] Créer un **module Auth** (JWT, sessions…)  
   - [x] Créer la table `users` avec rôles (CLIENT, BOULANGER, ADMIN)  
   - [x] Endpoints d'inscription / connexion (si besoin d'accès client protégé)  
   - [x] Tests unitaires et E2E de base  
...
2. **Endpoints pour la liste des produits** ⚪️
